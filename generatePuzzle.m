 function S = generatePuzzle(d)

S = zeros(9,9); %Creates martirx called 'S'
...
for k = i
puzzle = S(ref(ceil(I(k)/3),:),ref(ceil(J(k)/3),:));
check = [S(I(k),:) S(:,J(k))' puzzle(:)'];
difference = unique(check);
choice = setdiff(1:9,difference);
if isempty(choice)
break;
